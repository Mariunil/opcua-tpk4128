...
data from the OPC UA data model over UDP multicast using the UADP encoding.

The publisher uses high level Publisher-API.

The publisher is publishing, at a publishing rate of once every five seconds, the "temperature"
"read" from a fake sensor, here modelled as a double of 25.0 +/- 0.999.

The publisher contain the information model of the sensor, which is only containing the
